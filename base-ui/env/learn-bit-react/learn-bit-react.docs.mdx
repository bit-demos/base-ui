...
Under the **variant** section of your `workspace.json` file choose which components you want to have the custom environment set. You can find the id of the extension in the `my-react.aspect.ts` file.
...
Bit uses ESLint to lint your components. You can add your own rules in the `my-react.main.runtime` file.
...
Bit uses Prettier to format your components. You can add your own rules in the `my-react.main.runtime` file.
...
Override the default dependencies in the `my-react.main.runtime` file to include react types of a different version for example.
...
Extend the `my-react.preview.runtime` file when you want to add your own customizations only for previewing in the Bit UI.
